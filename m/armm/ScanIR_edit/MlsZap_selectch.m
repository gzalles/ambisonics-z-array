function [response, clipFlag] = mlsZap_selectch(outputChl, inputChl, srate, signalLength, responseLength, numRepeats, scale, savewav)
...
%% checking arguments
if nargin < 8, savewav = 0; end;
if nargin < 7, scale = .9; end;
if nargin < 6, numRepeats = 1; end;
if nargin < 5, responseLength = 2048; end;
if nargin < 4, signalLength = 15; end;
...
if responseLength>length(signal)
    disp(sprintf('The requested response length exceeds MLS period, maximum response length is: %i', length(signal)));
end
...
temp = playrec_selectch(psig', srate, outputChl, lic, (length(psig)+responseLength)/srate);
