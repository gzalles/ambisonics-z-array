...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%uncomment to run auto measure with stepper 
%todo: add check mark to "create new" figure/GUI
% 

% Arduino Stepper Motor rotating function
handles.a = arduino('/dev/tty.usbmodem146301', 'Uno', 'Libraries', 'Adafruit\MotorShieldV2');
%           arduino('/dev/tty.usbmodem1411', 'Uno', 'Libraries', 'Adafruit\MotorShieldV2');
%todo: add serial string port to GUI

% initialize arduino
handles.shield = addon(handles.a, 'Adafruit\MotorShieldV2');
handles.addrs = scanI2CBus(handles.a, 0);
handles.sm = stepper(handles.shield, 2, 100, 'stepType', 'Single');
handles.sm.RPM = 10;
%todo: add rpm and steps per rev (sprev = 100) to GUI.

%use this to allow user to exit room (0 = will pause)
handles.initialPause = 0; 
...
% --- Executes on selection change in ploape_popup.
...
if ( strcmpi (handles.specs.signalType, 'Sine Sweep') || strcmpi (handles.specs.signalType, 'Sine-Sweep'))
    y = sweepZap_selectch(handles.app.outchl,handles.app.numInputChls,handles.specs.sampleRate,handles.app.sigLength,recLen,handles.app.numPlays, 20,handles.specs.sampleRate/2, 1, savewav);
elseif ( strcmpi (handles.specs.signalType, 'MLS') )
    mlsLen = nextpow2(handles.app.sigLength * handles.specs.sampleRate + 1);
    y = MlsZap_selectch(handles.app.outchl,handles.app.numInputChls,handles.specs.sampleRate,mlsLen,recLen,handles.app.numPlays,1,savewav);
elseif ( strcmpi (handles.specs.signalType, 'Golay Codes') || strcmpi (handles.specs.signalType, 'Golay-Codes'))
    ...
    y = golayZap_selectch(handles.app.outchl, handles.app.numInputChls, handles.specs.sampleRate,golayLen,recLen,handles.app.numPlays,1,1,savewav);
end;
...
%add a pause to let user exit room - GZB - 2/24/2019
if (handles.initialPause == 0)
    disp("Auto Measure Pressed.")
    pause(30); %initial pause of 30 seconds
    handles.initialPause = 1;
end
