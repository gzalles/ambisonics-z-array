function [response, clipFlag] = golayZap(outputChl, inputChl, srate, signalLength, responseLength, numRepeats, delay, scale, savewav)
...
%% golay play back
SA = playrec_selectch(ones(loc,1)*golayPair(1,:), srate, outputChl, lic,recLength);
[a b] = size(SA);
...
pause(delay);
SB = playrec_selectch(ones(loc,1)*golayPair(2,:), srate, outputChl, lic,recLength);
[a b] = size(SB);
...
while i < numRepeats
    pause(delay)
    temp = playrec_selectch(golayPair(1,:)*ones(loc,1), srate, outputChl, lic,recLength);
    if (max(abs(temp)) > 0.95), clipFlag = 1.0; end;
    SA = SA+temp;
    pause(delay);
    temp = playrec_selectch(golayPair(2,:)*ones(loc,1), srate, outputChl, lic,recLength);
    if (max(abs(temp)) > 0.95), clipFlag = 1.0; end;
    SB = SB+temp;
    i = i+1;
end
